...
* Introduction
  ...
  It's friday afternoon, the end of the sprint, and a few hours before the weekend starts. The last Product Backlog Item (PBI) is in QA and while you wait for the feedback from QA (which you're almost sure will be positive) you sit relaxed while a vague plan for the weekend starts unfolding in your head.

  Suddenly, you get a notification --- there is an issue with the PBI. You jump on it to see what the problem is, and after discussing with the QA engineer you find out that the issue is due to some leftover data from the previous PBI.

  Having identified the problem, you spend a few minutes to craft a SQL script that will clean the data, and give it to the QA engineer. He/she runs the script on the QA database, starts testing the PBI from the beginning, and then confirms that the application is "back to normal".
